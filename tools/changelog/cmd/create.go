...
var createCmd = &cobra.Command{
	...
	Long:  `Creates changelog entries in a project. Example:`,
	RunE:  runCreate,
}
...
func runCreate(cmd *cobra.Command, args []string) error {
	...
	var subs []string
	if scope == "" {
		scope, subs, err = scopePrompt(config, subs)
	} else {
...
func renderTitle(date string, change changelog.Change, title string) string {
...
func init() {
	...
	typeUsage := fmt.Sprintf("Type of entry, as defined by config file")
	createCmd.Flags().StringVarP(&typ, "type", "t", "", typeUsage)

	scopeUsage := fmt.Sprintf("Scope of entry, as defined by config file, with optional\n" +
	...
	createCmd.Flags().StringVarP(&title, "title", "", "", "Title of file to create")
