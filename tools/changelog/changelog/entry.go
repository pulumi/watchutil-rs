...
func (cl *Changelog) Merge(other Changelog) Changelog {
	var changes []*Change
	changes = append(changes, cl.Changes...)
	changes = append(changes, other.Changes...)
	return Changelog{
		Changes: changes,
	}
}

// Change records a change by its category, the scope of the change, and metadata from GitHub needed to render a
// changelog.
//
...
func (v *Change) Conventional() string {
	var message string
